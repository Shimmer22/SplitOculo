...
|------|-------------|---------|------|
| Layer 4 (1280) | **0.86** | 0.63 | ⚠️ 流水线正常，语义质量待提升 |
| Layer 8 (1280) | ~0.77 | 1.07 | ✅ 可学习 |
| Merger (2048) | ~0.00 | ~4.8 | ❌ 太难 |

### 质量分析

- **cos_sim=0.86**: 流水线正常，但**语义信息丢失**
- **需要**: cos_sim > 0.95 才能保持语义正确，或端到端微调
- **真值测试**: Qwen Layer 4 特征可产生正确输出，CNN 特征不能
...
### 特征质量差距
当前训练达到 0.86 余弦相似度，**不足以保持语义正确性**。流水线经验证是正确的——使用 Qwen 真实的 Layer 4 特征时，LLM 能产生正确回复。

**可能的改进方向：**
- 使用更大/更多样化的数据集训练
- 使用更深或更宽的 CNN 架构
- 尝试端到端微调 Qwen blocks[4:]
- 目标 cos_sim > 0.95
